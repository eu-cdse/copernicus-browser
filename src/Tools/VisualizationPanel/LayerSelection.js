...
import { YYYY_MM_REGEX } from '../../Tools/SearchPanel/dataSourceHandlers/PlanetBasemapDataSourceHandler';
import { PLANET_NICFI } from '../../Tools/SearchPanel/dataSourceHandlers/dataSourceConstants';
import { sortLayers } from './VisualizationPanel.utils';
import store, { visualizationSlice } from '../../store';
import { parseEvalscriptBands, parseIndexEvalscript, usePrevious } from '../../utils';
...
function LayerSelection({
  ...
  function onSelectCustomVisualization() {
    ...
    store.dispatch(
      visualizationSlice.actions.setVisualizationParams({
        ...
        evalscript: datasourceHandler.generateEvalscript(selectedBands, datasetId),
      }),
    );
  }
  ...
  function onCompositeChange(bands) {
    store.dispatch(
      visualizationSlice.actions.setVisualizationParams({
        evalscript: datasourceHandler.generateEvalscript(bands, datasetId),
      }),
    );
  }
