...
const CollectionSelection = ({
  ...
  const onSelect = (selectedCollection, orbitDirection = null) => {
    ...
    const { dataset } = selectedConfig;
    if (dataset !== datasetId) {
      const dsh = getDataSourceHandler(datasetId);
      const sibling = dsh && dsh.getSibling(datasetId);
      const resetDates = sibling ? sibling.siblingId !== dataset : true;
      store.dispatch(
