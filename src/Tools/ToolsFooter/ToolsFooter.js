import React, { Component } from 'react';
import { connect } from 'react-redux';

import ExternalLink from '../../ExternalLink/ExternalLink';
import { VERSION_INFO } from '../../VERSION';
import { t } from 'ttag';

import { ReactComponent as EuropeBanner } from './icons/europe-banner.svg';
import { ReactComponent as EsaBanner } from './icons/esa-banner.svg';
import { ReactComponent as CopernicusBanner } from './icons/copernicus-banner.svg';
import { ReactComponent as VerticalLine } from './icons/vertical-line.svg';

import './ToolsFooter.scss';

class ToolsFooter extends Component {
  render() {
    // const branch = VERSION_INFO.branch ? ` ${VERSION_INFO.branch}` : null;
    const commit = VERSION_INFO.commit ? ` [${VERSION_INFO.commit.substring(0, 8)}]` : null;
    const tag =
      VERSION_INFO.tag.startsWith('CDSE') === true
        ? VERSION_INFO.tag.substring('CDSE-'.length)
        : VERSION_INFO.tag;

    return (
      <div className="tools-footer">
        <div className="footer-info">
          {/* {!this.props.user && (
            <div className="free-signup">
              <ExternalLink
                href={`${process.env.REACT_APP_AUTH_BASEURL}oauth/subscription?origin=EOBrowser&param_client_id=${process.env.REACT_APP_CLIENTID}`}
              >
                {t`Free sign up`}
              </ExternalLink>
              &nbsp;
              <span>{t`for all features`}</span>
            </div>
          )} */}
          <div className="credentials">
            <div className="banners">
              <ExternalLink className="europe-banner" href="https://commission.europa.eu/index_en">
                <EuropeBanner />
              </ExternalLink>
              <ExternalLink
                className="copernicus-banner"
                href="https://www.copernicus.eu/en/about-copernicus"
              >
                <CopernicusBanner />
              </ExternalLink>
              <ExternalLink className="esa-banner" href="https://www.esa.int/">
                <EsaBanner />
              </ExternalLink>
            </div>

            <div className="about-container">
              <VerticalLine />
              <ExternalLink className="about" href="https://dataspace.copernicus.eu">
                {t`About`}
              </ExternalLink>
              <VerticalLine />
            </div>

            <div className="powered-by">
              <span>{t`Powered by Sentinel Hub`}</span>
            </div>

            <div className="app-version">
              {VERSION_INFO.tag ? (
                <div>{tag}</div>
              ) : VERSION_INFO.commit ? (
                <div title={`${commit}`}>
                  {/* {branch} */}
                  {commit}
                </div>
              ) : (
                <div>Local build</div>
              )}
              {process.env.REACT_APP_REPLACE_SERVICES_HOSTNAME && (
                <div>{process.env.REACT_APP_REPLACE_SERVICES_HOSTNAME}</div>
              )}
            </div>
          </div>
        </div>
      </div>
    );
  }
}

const mapStoreToProps = (store) => ({
  user: store.auth.user.userdata,
});

export default connect(mapStoreToProps, null)(ToolsFooter);
