import {
  checkCCMProduct,
  checkCMMDownloadAccess,
  showCCMDownloadAccessErrormessage,
} from './ProductInfo.utils';

const productWithCCMAttribute = {
  attributes: [
    {
      '@odata.type': '#OData.CSC.StringAttribute',
      Name: 'collection',
      Value: 'CCM',
      ValueType: 'String',
    },
  ],
  productType: 'HRG_THX__3_56FB',
};

const productWithOutCCMAttribute = {
  attributes: [{}],
};

const userTokenWithCCMAccessRole =
  'eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICJYVUh3VWZKaHVDVWo0X3k4ZF8xM0hxWXBYMFdwdDd2anhob2FPLUxzREZFIn0..R_S74ACzhLSTfO_KOTHgCBk8vYxTDKN07Y4iiuAu6BC6FQkf6V9q1hXriR56mSqS-FNl84SVLrNLlN6ydQpNhID4VecvslRippKRSq6hX54slYDz4UG0eu-n4ZnCDYNjT61V4SqUBW-cCZEBI53MNlkBEHtOwZ-P66CKjKJCka8EZjHvtV9j3OAAbvaqLyBcrWo3DrD_0zvMWY0QUmKxXemt6j2O9OQpqQJohIlcsADLIuZfDRStffxNhVPfhHl2nqCiOjvah0H-lU7FDXrHI7QB4VpRgGi6aKFRimYMIpi-NvrUo516ydBoyb_fnUNWu3tzyBdnhna177z4MCpN-w';

const userTokenWithOutCCMAccessRole =
  'eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICJYVUh3VWZKaHVDVWo0X3k4ZF8xM0hxWXBYMFdwdDd2anhob2FPLUxzREZFIn0..GC_CxDLeguOtZRhKTWfVl-PoxbQS35DZSUKslaAIb3MVr5KV1f2HeGaw_HnGATiFAQvLBzch_bAiTut0yOZUL1Rtrhft39LQrkQIvCXRM3E0GR8P9qKMvPtTm7F9NlPtYmkCkDJ_h9xQEzVfs2SIWxdwz7uDiLLwsKTVkUEP9GzpHJaTT6BuFxMWkl_BemWPZY_QZ6GqRddFc72yZJ2JFj8VlqXv61QdK-ezCg5hU_WyfbzQhI6o3ROLJZ_T-GLyGfWyqHVYp9e7VVqRgGM2WRd089aNPc5AMoDQD26OJFRusQcL2GoC_TpXzCioCJd_Pcd2ZZj3S-ak4KM0IW4ZIA';

describe('test functions responsible for validating CCM product and access role, as well error message', () => {
  test('checkCCMProduct', () => {
    expect(checkCCMProduct(productWithCCMAttribute)).toBeTruthy();
    expect(checkCCMProduct(productWithOutCCMAttribute)).toBeFalsy();
  });

  test('checkCMMDownloadAccess', () => {
    const { productType } = productWithCCMAttribute;

    expect(checkCMMDownloadAccess(userTokenWithCCMAccessRole, { productType })).toBeTruthy();
    expect(checkCMMDownloadAccess(userTokenWithOutCCMAccessRole, { productType })).toBeFalsy();
  });

  test('showCCMDownloadAccessErrormessage', () => {
    // User without CCM access role
    expect(
      showCCMDownloadAccessErrormessage(userTokenWithOutCCMAccessRole, productWithCCMAttribute),
    ).toBeTruthy();
    expect(
      showCCMDownloadAccessErrormessage(userTokenWithOutCCMAccessRole, productWithOutCCMAttribute),
    ).toBeFalsy();

    // User with CCM access role
    expect(
      showCCMDownloadAccessErrormessage(userTokenWithCCMAccessRole, productWithCCMAttribute),
    ).toBeFalsy();
    expect(
      showCCMDownloadAccessErrormessage(userTokenWithCCMAccessRole, productWithOutCCMAttribute),
    ).toBeFalsy();
  });
});
