...
class ThemesProvider extends React.Component {
  ...
  async componentDidUpdate(prevProps) {
    ...
    if (
      ...
      const selectedMode = this.guessMode(this.props.themeIdFromUrlParams);
      this.setMode(selectedMode);
      this.setSelectedThemeIdFromMode(selectedMode);

      const allThemes = [...userInstances, ...selectedMode.themes];

      const isThemeIdInModeThemesList = !!allThemes.find((t) => t.id === this.props.themeIdFromUrlParams);

      if (selectedMode.themes.length > 0 && !isThemeIdInModeThemesList && this.props.themeIdFromUrlParams) {
  ...
  async fetchUserInstances() {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_SH_SERVICES_URL}/configuration/v1/wms/instances`,
        {
          responseType: 'json',
          headers: {
      ...
      const modifiedUserInstances = response.data.map((inst) => ({
        ...
        content: [
          {
            service: 'WMS',
            url: `${process.env.REACT_APP_SH_SERVICES_URL}/ogc/wms/${inst.id}`,
          },
        ],
      }));
