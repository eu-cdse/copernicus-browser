...
export async function fetchAndPatchImagesFromParams(params, setWarnings, setError, setLoadingImages) {
  ...
  for (let idx = comparedLayers.length - 1; idx >= 0; idx--) {
    ...
    try {
      ...
      if (dsh) {
        drawSHLogo = dsh.isSentinelHub() || drawSHLogo;
        drawCopernicusLogo = dsh.isCopernicus() || drawCopernicusLogo;
        addLogos = drawSHLogo || drawCopernicusLogo || addLogos;
      }
...
export async function fetchImageFromParams(params, raiseWarning) {
  ...
  let addLogos = false;
  if (dsh) {
    drawSHLogo = dsh.isSentinelHub();
    drawCopernicusLogo = dsh.isCopernicus();
    addLogos = drawSHLogo || drawCopernicusLogo;
  }
...
export async function addImageOverlays(
  ...
  if (showLogo) {
    ...
    await drawLogos(ctx, logosPartitionWidth, getLowerYAxis(ctx), drawSHLogo, drawCopernicusLogo);
  }
  if (drawAoiGeoToImg) {
...
export const drawCaptions = async (
  ...
  if (logos) {
    await drawLogos(ctx, logosPartitionWidth, bottomYAxis, drawSHLogo, drawCopernicusLogo);
  }
  if (copyrightText) {
...
async function drawLogos(ctx, logosPartitionWidth, bottomY, drawSHLogo, drawCopernicusLogo) {
  if (!drawSHLogo && !drawCopernicusLogo) {
    return;
  }

  let sentinelHubLogo;
  if (drawSHLogo) {
    sentinelHubLogo = await loadImage(SHlogo);
  }
  ...
  const ratio = proposedWidth / (drawSHLogo ? sentinelHubLogo.width : copernicusLogo.width);
  ...
  const sentinelHubLogoWidth = drawSHLogo ? sentinelHubLogo.width * ratio : 0;
  const sentinelHubLogoHeight = drawSHLogo ? sentinelHubLogo.height * ratio : 0;
