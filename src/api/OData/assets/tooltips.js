...
const productAvailabilityTooltip = t`The product availability indicates whether a product can be downloaded immediately (*Immediate*) or must first be processed in the user's workspace (*To order*). The workspace offers processing options for:
...
const S1 = {
  [AttributeNames.platformSerialIdentifier]: t`The available Sentinel-1 data includes products from the Sentinel-1 satellite constellation, including Sentinel-1A (operational since 2014) and Sentinel-1B (operational from 2016 to 2022).`,
  [AttributeNames.orbitDirection]: t`Determines whether the data was recorded during a descending orbit (flight direction: north - south) or an ascending orbit (flight direction: south - north).`,
  ...
  [AttributeNames.operationalMode]: t`Sentinel-1 operates in 4 exclusive instrument acquisition modes. Depending on the instrument mode, the data products may be available in different polarisation, resolution and swath withs (between 20 km and 410 km). The four different modes are:
- Stripmap (SM) 
- Interferometric Wide swath (IW)
- Extra-Wide swath (EW)
- Wave (WV)
  `,
  [AttributeNames.swathIdentifier]: t`Depending on the acquistion mode, the following swaths and identifiers are possible:
- S1-S6 for SM mode (currently the only supported option)
- IW1-IW3 for IW mode
- EW1-EW5 for EW mode
- WV1-WV2 for WV mode
  `,
  [AttributeNames.polarisationChannels]: t`Determines with which polarisation the data was acquired. The first letter indicates the polarisation when sending the signal, the second letter when receiving the signal, e.g., HV = horizontal polarisation when sending the signal and vertical polarisation when receiving.
...
const S2 = {
  [AttributeNames.platformSerialIdentifier]: t`The available Sentinel-2 data includes products from the Sentinel-2 satellite constellation, including Sentinel-2A (operational since 2015) and Sentinel-2B (operational since  2017).`,
  ...
  [AttributeNames.origin]: t`The available Sentinel-2 data is processed either by ESA or by CloudFerro.`,
  [AttributeNames.online]: productAvailabilityTooltip,
  [AttributeNames.S2Collection]: t`Sentinel-2 Collection 1 represents the product dataset generated starting from processing baseline 05.XX included.`,
};
...
- Non Time Critical (NTC): typically available within 1 months after data acquisition`,
  [AttributeNames.platformSerialIdentifier]: t`The available Sentinel-3 data includes products from the Sentinel-3 satellite constellation, including Sentinel-3A (operational since 2016) and Sentinel-3B (operational since 2018).`,
  [AttributeNames.orbitDirection]: t`Determines whether the data was recorded during a descending orbit (flight direction: north - south) or an ascending orbit (flight direction: south - north).`,
...
const S5P = {
  [AttributeNames.processingMode]: t`The timeliness provides information about the delivery time of the data product. For Sentinel-5P there are 3 different timeliness available:
...
- Reprocessing: no time constraints (performed when major product upgrades are required`,
  [AttributeNames.orbitNumber]: t`The absolute Orbit Number is the number of orbits that have elapsed since the first ascending node crossing after launch.`,
};
