...
async function fetchInstances(client) {
  ...
  const instances = response.data
    ...
    .filter((instanceId) => DEFAULT_THEMES[0].content.find((themes) => themes.url.indexOf(instanceId) > -1));
  return instances;
}
...
function removeExisting(instances) {
  ...
  const knownInstances = DEFAULT_THEMES[0].content.map((theme) => theme.url.split('/').pop());
